...
SERIES_FORMAT="Series/{n}/{episode.special ? 'Special' : 'Season '+s.pad(2)}/{n} - {episode.special ? 'S00E'+special.pad(2) : s00e00} - {t.${QUOTE_FIXER}.replaceAll(/[!?.]+$/).replacePart(', Part $1')}{'.'+lang}"
...
filebot -script fn:amc -no-xattr --lang fr --output /media --log-file /files/amc.log --action move --conflict auto \
